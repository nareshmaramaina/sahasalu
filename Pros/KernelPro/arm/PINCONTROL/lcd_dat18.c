...
#define MX6_BRD_LCD_DAT18	IMX_GPIO_NR(3,6)

static int __init gsmusbpwr_init(void)
{
	mxc_iomux_v3_setup_pad(MX6SL_PAD_LCD_DAT18__GPIO_3_6) ;
	gpio_request(MX6_BRD_LCD_DAT18, "gsmusb_pwr");
        gpio_direction_output(MX6_BRD_LCD_DAT18, 0);
	gpio_set_value(MX6_BRD_LCD_DAT18, 1);
...
static void __exit gsmusbpwr_exit(void)
{
	gpio_set_value(MX6_BRD_LCD_DAT18,0);
//	gpio_set_value(MX6_BRD_ECSPI2_SS0, 0);
//	gpio_set_value(MX6_BRD_LCD_DATA19, 0);
//	gpio_set_value(MX6_BRD_SD2_DAT5, 0);
}
module_init(gsmusbpwr_init);
module_exit(gsmusbpwr_exit);

...
#define MX6_BRD_EPDC_PWRSTAT	IMX_GPIO_NR(2,13)

static int __init camerapwr_init(void)
{
	mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_PWRSTAT__GPIO_2_13) ;
	gpio_request(MX6_BRD_EPDC_PWRSTAT, "camera_pwr");
        gpio_direction_output(MX6_BRD_EPDC_PWRSTAT, 0);
	gpio_set_value(MX6_BRD_EPDC_PWRSTAT, 1);
...
static void __exit camerapwr_exit(void)
{
	gpio_set_value(MX6_BRD_EPDC_PWRSTAT,0);
//	gpio_set_value(MX6_BRD_ECSPI2_SS0, 0);
//	gpio_set_value(MX6_BRD_LCD_DATA19, 0);
//	gpio_set_value(MX6_BRD_SD2_DAT5, 0);
}
module_init(camerapwr_init);
module_exit(camerapwr_exit);

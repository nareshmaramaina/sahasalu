...
#define MX6_BRD_KEY_ROW3	IMX_GPIO_NR(3,31)

static int __init sampwr_init(void)
{
	mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_ROW3__GPIO_3_31) ;
	gpio_request(MX6_BRD_KEY_ROW3, "sam_pwr");
        gpio_direction_output(MX6_BRD_KEY_ROW3, 0);
	gpio_set_value(MX6_BRD_KEY_ROW3, 1);
...
static void __exit sampwr_exit(void)
{
	gpio_set_value(MX6_BRD_KEY_ROW3,0);
//	gpio_set_value(MX6_BRD_ECSPI2_SS0, 0);
//	gpio_set_value(MX6_BRD_LCD_DATA19, 0);
//	gpio_set_value(MX6_BRD_SD2_DAT5, 0);
}
module_init(sampwr_init);
module_exit(sampwr_exit);

...
void sig_handler(int signo)
{
		...
		stop_flag = 1;
//fprintf(stdout,"\n\033[1;31 Sending  stop signal to last foreground process, Please wait few secs(With in 60secs) and also you will get last command stdout\033[0m\n");
		fprintf(stdout,"\n \033[1;31m Please wait few secs ...,and also you will get last command status\033[0m\n");
/*	if (Foreground == 1)
	{
//		Foreground = 0;
...
repeat:		ret = wait_for_data(client_sockfd);
		...
		fflush(stdout);	
			...
			close(sockfd);
			printf("\e[2J\e[H"); // ctrl + l
			printf("\n\033[1;31mWaiting for new incoming connections..., On Port Number = %d\n", PORT);
			printf("\033[0m"); // reset colour
			return 0;
		}

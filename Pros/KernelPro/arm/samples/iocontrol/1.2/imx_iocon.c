...
#include"/home/arun/linux-3.0.35_dop/arch/arm/plat-mxc/include/mach/iomux-mx6sl.h"
...
//	printk("\narg.........:%d\n",(unsigned int)arg);
	switch(cmd)
	{
		...
		case GSM_PWR_CNTRL:
			...
			if((unsigned int)arg==1) //Modified by BINDU on 05/09/2015.
			{
	
				gpio_set_value(GSM_M_RESET_PIN,0); //Making Reset pin Low because Default State is Low.
		...
		case GSM_M_RESET:
			...
			gpio_set_value(GSM_M_RESET_PIN,0); //Delay Modified as per datasheet-Default delay is 200ms
	                mdelay(100);
                        gpio_set_value(GSM_M_RESET_PIN,1); //Modified by BINDU on 05/09/2015.
                        mdelay(200);

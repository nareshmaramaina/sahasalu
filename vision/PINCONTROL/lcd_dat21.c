...
#define MX6_BRD_LCD_DAT21	IMX_GPIO_NR(3,9)

static int __init gsmpwr_init(void)
{
	mxc_iomux_v3_setup_pad(MX6SL_PAD_LCD_DAT21__GPIO_3_9) ;
	gpio_request(MX6_BRD_LCD_DAT21, "gsm_pwr");
        gpio_direction_output(MX6_BRD_LCD_DAT21, 0);
	gpio_set_value(MX6_BRD_LCD_DAT21, 1);
...
static void __exit gsmpwr_exit(void)
{
	gpio_set_value(MX6_BRD_LCD_DAT21,0);
//	gpio_set_value(MX6_BRD_ECSPI2_SS0, 0);
//	gpio_set_value(MX6_BRD_LCD_DATA19, 0);
//	gpio_set_value(MX6_BRD_SD2_DAT5, 0);
}
module_init(gsmpwr_init);
module_exit(gsmpwr_exit);

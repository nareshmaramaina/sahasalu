...
//#define WI_FI_PWR_ENABLE        0xc7 
//#define RF_ENABLE		0xc8
...
//Added this API by Himabindu on 13/11/2017 For External USB Host1.
int gl11_extusb1_control(int arg)
{
        int ret=0;
	...
	//printf("Ext USB1 \n");
        ...
        if(arg == 1)
        {
                ...
                ret = ioctl(fd,EXT_USB1,TRUE);
                return ret;
        }
        else if(arg == 0)
        {
                ret = ioctl(fd,EXT_USB1,FALSE);
                return ret;
        }
        else
        {
...
//Added this API by Himabindu on 13/11/2017 For External USB Host2.
int gl11_extusb2_control(int arg)
{
	return -1;
	...
	//printf("Ext USB2 :%d\n",arg);
        ...
        if(arg == 1)
        {
                ret = ioctl(fd,EXT_USB2,TRUE);
                return ret;
        }
        else if(arg == 0)
        {
                ret = ioctl(fd,EXT_USB2,FALSE);
                return ret;
        }
        else
        {
...
int gl11_wifi_control(int val)
{
        ...
        if(val==0||val==1)
        {
                if(val)
                {
                        ret=ioctl(fd,RFU_USB_PWR_CNTRL,TRUE);
                        return ret;
                }
                else
                {
                        ret=ioctl(fd,RFU_USB_PWR_CNTRL,FALSE);
                        return ret;
                }
        }
        else
...
int gl11_wifi_rfcontrol(int val)
{
        int ret=0;
        ...
        if(val==0||val==1)
        {
                if(val)
                {
                        ret=ioctl(fd,WIFI_RF_CNTRL,TRUE);
                        return ret;
                }
                else
                {
                        ret=ioctl(fd,WIFI_RF_CNTRL,FALSE);
                        return ret;
                }
        }
        else
...
int gl11_gps_pwr_on(void)
{

        return ioctl(fd, GPS_PWR_CNTRL, TRUE);

	return 0;
}
int gl11_gps_pwr_off(void)
{
        return ioctl(fd, GPS_PWR_CNTRL, FALSE);
...
int gl11_modem_reset(void)
{
	...
	ret = ioctl(fd, GSM_M_RESET, TRUE);
	//printf("GSM Modem Reset return Value:%d\n",ret);	
	return ret;
}
...
int gl11_GetBatteryInfo(int *AdapPresent,int *BattSt,int *BattVt)
{
	...
	*AdapPresent = (adpt >1000)?1:0;
	...
	if(adpt > 1000){
		...
		ret=ioctl(fd,BATTER_SENSE,&get_batterysense);

		*BattSt = get_batterysense.batsense_val;
		//	printf("BattStatus is %d\n",get_batterysense.batsense_val);
	}
	else{
		*BattSt = 0;
	}
...
int gl11_audio_vol_control(int val)
{
	...
	if(val< 130 || val>190)
		return -1;
	printf("Audio Volume is %d\n",val);	
	sprintf(buff,"amixer cset numid=1 %d",val);
	if(system(buff) != 0){

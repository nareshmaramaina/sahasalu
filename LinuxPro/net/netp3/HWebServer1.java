 ...
 * as comments in the source code. Asimple webserver
 * implemented by 
 *"we don't know author's name"
 */
import java.io.*;
import java.net.*;
import java.util.*;
class HWebServer1 implements HttpConstants {
...
class Worker extends HWebServer1 implements HttpConstants, Runnable {
    ...
    public synchronized void run() {
        while(true) {
             ...
             */
            s = null;
            Vector pool = HWebServer1.threads;
            synchronized (pool) {
                if (pool.size() >= HWebServer1.workers) {
    ...
    void handleClient() throws IOException {
         ...
         */
       // s.setSoTimeout(HWebServer1.timeout);
...
outerloop:
	...
	//	 log(rem);	
            ...
            File targ = new File(HWebServer1.root, fname);
            if (targ.isDirectory()) {
